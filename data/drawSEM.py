...
# ================= 字体设置 =================
NODE_FONT_SIZE = '30'  # 节点字体
EDGE_FONT_SIZE = '26'  # 边标签字体
OBSERVED_FONT_SIZE = '28'  # 观测变量字体
...
for name, attrs in latent_nodes.items():
    sem_diagram.node(
        name, shape='circle', width='0.5', height='0.5',
...
for var, label in observed_vars.items():
    sem_diagram.node(
        ...
        width='1.2', height='0.8', fontsize=OBSERVED_FONT_SIZE,
        penwidth='1.2'
    )
...
for e in errors:
    sem_diagram.node(
        e, shape='circle', width='0.3', height='0.3',
...
for err, var, coef in error_edges:
    sem_diagram.edge(
        ...
        color=ERROR_COLOR, penwidth='1.6', arrowsize='0.6'
    )
